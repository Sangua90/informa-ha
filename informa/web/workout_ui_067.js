...
(function(){
  function decorateWorkout(){
    ...
    const total=cards.length;let flow={};try{flow=JSON.parse(localStorage.getItem('informha_workout_flow_0949')||'{}')||{}}catch(e){}const completedExercises=cards.filter(c=>{const id=c.id==='if50ex_cardio'?'cardio':String(c.id||'').replace('if50ex_',''),s=flow[id];return s?.stage==='archived'||c.classList.contains('if949-archived')}).length;const pct=total?Math.round(completedExercises/total*100):0;const segments=cards.map((c,i)=>{const id=c.id==='if50ex_cardio'?'cardio':String(c.id||'').replace('if50ex_',''),s=flow[id],status=s?.status||'',archived=s?.stage==='archived'||c.classList.contains('if949-archived'),cls=archived?(status==='Completato'?'done':status==='Parziale'?'partial':status==='Saltato'?'skipped':'done'):(c.classList.contains('if950-current')||localStorage.getItem('informha_workout_active_0109')===id?'active':'');return `<i class="${cls}" title="Esercizio ${i+1}${status?' · '+status:''}"></i>`}).join('');
  ...
  const css=document.createElement('style');css.textContent=`
    ...
    .if67-progress{height:10px;background:#090b0e;border:1px solid var(--ln);border-radius:999px;overflow:hidden;margin:12px 0}.if67-progress.if67-segmented{display:flex;gap:3px;padding:2px;height:14px;overflow:visible}.if67-progress.if67-segmented i{display:block;flex:1;height:8px;background:rgba(255,255,255,.10);border-radius:999px;transition:background .2s,box-shadow .2s,transform .2s}.if67-progress.if67-segmented i.done{background:#22c55e;box-shadow:0 0 10px rgba(34,197,94,.30)}.if67-progress.if67-segmented i.partial{background:#f59e0b;box-shadow:0 0 10px rgba(245,158,11,.25)}.if67-progress.if67-segmented i.skipped{background:#ef4444;box-shadow:0 0 10px rgba(239,68,68,.22)}.if67-progress.if67-segmented i.active{background:var(--blue);box-shadow:0 0 12px rgba(56,189,248,.38);transform:scaleY(1.25)}.if67-pct{font-size:22px;font-weight:900;color:var(--green2)}
