...
(function(){
  function decorateWorkout(){
    ...
    const total=cards.length;let flow={};try{flow=JSON.parse(localStorage.getItem('informha_workout_flow_0949')||'{}')||{}}catch(e){}const ids=cards.map(c=>c.id==='if50ex_cardio'?'cardio':String(c.id||'').replace('if50ex_',''));const completedExercises=ids.filter(id=>flow[id]?.stage==='archived').length;const pct=total?Math.round(completedExercises/total*100):0;let activeId=localStorage.getItem('informha_workout_active_0109');if(activeId&&flow[activeId]?.stage==='archived')activeId=null;const segments=cards.map((c,i)=>{const id=ids[i],s=flow[id],status=s?.status||'',archived=s?.stage==='archived',cls=archived?(status==='Completato'?'done':status==='Parziale'?'partial':status==='Saltato'?'skipped':status==='Sostituito'?'replaced':'done'):(activeId===id?'active':'');return `<i class="${cls}" title="Esercizio ${i+1}${status?' · '+status:''}"></i>`}).join('');
