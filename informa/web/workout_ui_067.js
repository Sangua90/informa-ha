...
(function(){
  function decorateWorkout(){
    ...
    const total=cards.length;let flow={};try{flow=JSON.parse(localStorage.getItem('informha_workout_flow_0949')||'{}')||{}}catch(e){}const completedExercises=Object.values(flow).filter(s=>s?.stage==='archived').length;const pct=total?Math.round(completedExercises/total*100):0;const segments=cards.map((c,i)=>{const id=c.id==='if50ex_cardio'?'cardio':String(c.id||'').replace('if50ex_',''),s=flow[id],status=s?.status||'',archived=s?.stage==='archived'||c.classList.contains('if949-archived'),cls=archived?(status==='Completato'?'done':status==='Parziale'?'partial':status==='Saltato'?'skipped':status==='Sostituito'?'replaced':'done'):(c.classList.contains('if950-current')||localStorage.getItem('informha_workout_active_0109')===id?'active':'');return `<i class="${cls}" title="Esercizio ${i+1}${status?' · '+status:''}"></i>`}).join('');
