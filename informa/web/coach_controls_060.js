...
function if60ToPlan(id){const x=if60Library(id);if(!x)return null;const reps=x.reps??(x.repType==='seconds'?30:10);return {id,name:x.name,priority:x.priority||'Utile',sets:x.sets||3,reps,rest:x.rest||90,repType:x.repType||'reps',loadType:x.loadType||'bodyweight',guide:x.guide||null,cardio:!!x.cardio,duration:x.duration||null,equipment:x.equipment||'',group:x.group||''}}
