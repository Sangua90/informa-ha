...
(function(){
 ...
 window.if50ExerciseCard=function(ex){
  if(ex?.mobility||ex?.stretching){const sec=ex.seconds||90,label=ex.mobility?'Mobilità dinamica':'Stretching finale';return `<div class="card if50-ex if978-extra" id="if50ex_${ex.id}"><div class="ey">${label} · circa ${Math.round(sec/60*10)/10} min</div><h2>${ex.name}</h2><div class="sub">${ex.mobility?'Preparazione dinamica mirata alla seduta di oggi.':'Allungamento finale scelto da iCoach per i gruppi allenati.'}</div>${ex.guide?`<button class="btn secondary" onclick="openGuide('${ex.guide}')">Guida</button>`:''}<div class="if978-timer" id="if978timer_${ex.id}"><b>${sec} s</b><span>Tempo esercizio</span></div><div class="choice if978-timed-controls"><button class="if978-start" onclick="if978Start('${ex.id}',${sec},this)">▶ Avvia esercizio</button>${ex.stretching?`<button onclick="if50Status('${ex.id}','Saltato','Opzionale',this)">Salta</button>`:''}</div></div>`}return card0.apply(this,arguments)};
 ...
 window.if978Start=function(id,sec,btn){
  ...
  if978Paused=false;if978Left=Math.max(1,Number(sec)||90);if978EndAt=Date.now()+if978Left*1000;let left=if978Left,box=document.getElementById('if978timer_'+id);btn.dataset.running='1';btn.textContent='⏸ Pausa';
