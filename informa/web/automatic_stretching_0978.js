...
(function(){
 ...
 function paint(id){
  ...
  card.querySelectorAll('[data-timed-side]').forEach((b,i)=>{
   ...
   if(running){const pct=Math.max(0,Math.min(1,state.left/Math.max(1,Number(state.ex.seconds)||90))),dash=(201*(1-pct)).toFixed(1);b.innerHTML='<span>'+label+' — '+(state.paused?'Riprendi':'Pausa')+'</span><span class="if978-ring" aria-label="'+state.left+' secondi rimanenti"><svg viewBox="0 0 72 72" aria-hidden="true"><circle class="if978-ring-bg" cx="36" cy="36" r="32"></circle><circle class="if978-ring-progress" cx="36" cy="36" r="32" style="stroke-dashoffset:'+dash+'"></circle></svg><b>'+state.left+'<small>s</small></b></span>'}else b.textContent=label+' — '+(done?'✓ Completato':'▶ Avvia · '+state.ex.seconds+' s');
  });
