...
(function(){
 ...
 window.if978Start=function(id,sec,btn){
  ...
  if978Timer=setInterval(async()=>{left--;draw();if(left<=0){clearInterval(if978Timer);if978Timer=null;if(box)box.innerHTML='<b>✓</b><span>Esercizio completato</span>';btn.textContent='✓ Completato';try{await if50Status(id,'Completato','Opzionale',btn)}catch(e){}setTimeout(()=>window.if978Advance?.(id),350)}},1000)
 };
