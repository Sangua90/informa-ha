// InFormha 0.6.1 - rimozione tutorial e nuova libreria Esercizi in Altro
(function(){
  ...
  function exerciseList(){
    const lib=window.IF51_LIBRARY||{};
    const entries=Object.entries(lib).filter(([id,x])=>!x.cardio);
    const groups={};
    ...
    return Object.entries(groups).sort((a,b)=>a[0].localeCompare(b[0])).map(([group,items])=>`
      <div class="card exercise-library-group">
        ...
        ${items.map(([id,x])=>`<button class="exercise-library-item" onclick="if61OpenExercise('${id}')"><span><b>${x.name}</b><small>${x.equipment||''}</small></span><span>›</span></button>`).join('')}
      </div>`).join('');
  }

  window.if61OpenExercise=function(id){
    const x=(window.IF51_LIBRARY||{})[id]; if(!x)return;
    ...
    const guide=x.guide&&typeof openGuide==='function';
    page.innerHTML=`<div class="ey">Libreria esercizi</div><h1>${x.name}</h1>
      <div class="card"><div class="measure"><span>Gruppo</span><b>${x.group||'—'}</b></div><div class="measure"><span>Attrezzatura</span><b>${x.equipment||'—'}</b></div><div class="measure"><span>Serie base</span><b>${x.sets||'—'}</b></div><div class="measure"><span>Ripetizioni base</span><b>${x.reps||'—'}</b></div><div class="measure"><span>Recupero</span><b>${x.rest?x.rest+' sec':'—'}</b></div></div>
      ${guide?`<button class="btn" onclick="openGuide('${x.guide}')">Apri guida esercizio</button>`:''}
      <button class="btn secondary" onclick="go('exercises')">Torna agli esercizi</button>`;
    go('exercise-detail');
  };

  function installExercisesPage(){
    if(document.querySelector('[data-page="exercises"]'))return;
    const page=document.createElement('section');page.className='page';page.dataset.page='exercises';
    page.innerHTML=`<div class="ey">Altro</div><h1>Esercizi</h1><div class="sub" style="margin-bottom:14px">Libreria completa degli esercizi disponibili in InFormha, divisi per gruppo muscolare.</div>${exerciseList()}<button class="btn secondary" onclick="go('profile')">Indietro</button>`;
    document.querySelector('.app')?.insertBefore(page,document.querySelector('.nav'));

    const profile=document.querySelector('[data-page="profile"]');
    if(profile&&!document.getElementById('if61ExercisesButton')){
      const firstCard=profile.querySelector('.card');
      const b=document.createElement('button');b.id='if61ExercisesButton';b.className='btn secondary';b.textContent='🏋️ Esercizi';b.onclick=()=>go('exercises');
  ...
  removeTutorialUI();installExercisesPage();
})();
