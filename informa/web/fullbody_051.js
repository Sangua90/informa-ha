...
const IF51_LIBRARY={
 chest_press:{name:'Chest press alla macchina',group:'Petto',equipment:'Fassi',priority:'Essenziale',sets:3,reps:10,rest:120,guide:'chest'},
 lat_machine:{name:'Lat machine al petto',group:'Schiena',equipment:'Fassi + barra larga',priority:'Essenziale',sets:3,reps:10,rest:120,guide:'lat'},
 triceps_pushdown:{name:'Push-down tricipiti con corda',group:'Tricipiti',equipment:'Fassi + corda',priority:'Utile',sets:3,reps:12,rest:75,guide:'pushdown'},
 cable_curl:{name:'Curl bicipiti al cavo basso',group:'Bicipiti',equipment:'Fassi + barra corta',priority:'Utile',sets:3,reps:12,rest:75,guide:'curl'},
 ...
 leg_extension:{name:'Estensione ginocchio alla macchina (se disponibile)',group:'Gambe',equipment:'Fassi',priority:'Utile',sets:2,reps:12,rest:90,painAvoid:['Ginocchio']},
...
const IF51_PLANS={
 A:{title:'Spinta + Gambe',ids:['chest_press','shoulder_press','goblet_squat','triceps_pushdown','lateral_raise','calf_raise','treadmill']},
 B:{title:'Trazione + Catena posteriore',ids:['lat_machine','seated_row','romanian_deadlift','cable_curl','face_pull','plank','stepper']},
 C:{title:'Full body tecnico',ids:['chest_press','lat_machine','glute_bridge','shoulder_press','cable_curl','triceps_pushdown','plank','treadmill']}
};
...
function if51Home(){
 ...
 const card=document.createElement('div');card.id='if51HomeCard';card.className='card coach';const next=if51NextPlan();card.innerHTML=`<div class="ey">Programmazione flessibile</div><h2>Prossima rotazione: ${next} · ${IF51_PLANS[next].title}</h2><div class="sub">Nessun lunedì/mercoledì/venerdì obbligatorio: InFormha riparte dalla prossima seduta utile in base a ciò che hai realmente completato.</div><div class="measure"><span>Stato coach</span><b>${if51CalibrationNote()}</b></div>`;home.appendChild(card);
}
function if51PatchGenerate(){
 ...
 window.if50BuildPlan=function(){
  ...
  const key=IF50.intent==='Portami avanti'?if51NextPlan():if51NextPlan();
