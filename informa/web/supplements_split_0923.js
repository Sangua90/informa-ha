...
(function(){
  ...
  function editForm(item){
    item=item||{};
    return `<div class="card if923-editor" id="if923Editor"><div class="ey">${item.id?'Modifica':'Nuovo integratore'}</div><h2>${item.id?esc(item.name):'Impostazioni integratore'}</h2><input type="hidden" id="if923Id" value="${item.id||''}"><input class="field" id="if923Name" placeholder="Nome integratore" value="${esc(item.name||'')}"><input class="field" id="if923Type" placeholder="Tipologia (es. vitamina, minerale, performance...)" value="${esc(item.type_text||'')}" style="margin-top:8px"><div class="grid2" style="margin-top:8px"><input class="field" id="if923Dose" placeholder="Dose" inputmode="decimal" value="${item.dose??''}"><input class="field" id="if923Unit" placeholder="Unità (g, mg, cps...)" value="${esc(item.unit||'')}"></div><div class="ey" style="margin-top:14px">Giorni</div><div class="if923-days" id="if923Days">${dayChecks(item.days)}</div><div class="grid2" style="margin-top:10px"><input class="field" id="if923Time" type="time" value="${esc(item.time_text||'')}"><label class="if923-active"><input type="checkbox" id="if923Active" ${item.active===0?'':'checked'}> Attivo</label></div><input class="field" id="if923Notes" placeholder="Note (opzionale)" value="${esc(item.notes||'')}" style="margin-top:8px"><button class="btn" onclick="if923SaveSettings()">Salva</button><button class="btn secondary" onclick="if923CloseEditor()">Annulla</button></div>`;
  }
  ...
  window.if923SaveSettings=async()=>{const name=document.getElementById('if923Name')?.value.trim();if(!name){toast('Inserisci il nome');return}const days=[...document.querySelectorAll('#if923Days input:checked')].map(x=>Number(x.value));if(!days.length){toast('Seleziona almeno un giorno');return}const payload={id:Number(document.getElementById('if923Id')?.value)||null,name,type_text:document.getElementById('if923Type')?.value||null,dose:document.getElementById('if923Dose')?.value||null,unit:document.getElementById('if923Unit')?.value||null,days,time_text:document.getElementById('if923Time')?.value||null,notes:document.getElementById('if923Notes')?.value||null,active:!!document.getElementById('if923Active')?.checked};try{await api('api/supplements/settings-v2',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});toast('Impostazioni salvate');await loadSettings()}catch(e){toast(e.message||'Errore salvataggio')}};
  ...
  window.if923EnsureSettingsPage=ensureSettingsPage;
  window.if923LoadSettings=loadSettings;

  const oldGo=window.go;if(oldGo)window.go=function(page){const r=oldGo.apply(this,arguments);if(page==='supplements')setTimeout(loadDaily,25);if(page==='profile')setTimeout(wireProfileButton,25);if(page==='supplement-settings-0923')setTimeout(loadSettings,25);return r};
  ...
  ensureSettingsPage();wireProfileButton();setInterval(wireProfileButton,1200);if(location.hash==='#supplements')setTimeout(loadDaily,100);
})();
