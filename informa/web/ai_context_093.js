...
(function(){
  ...
  window.if93LoadContext=async function(){
    ...
    try{
      ...
      box.innerHTML=`<div class="grid2"><div class="metric"><span>Sedute recenti</span><b>${s.recent_sessions||0}</b></div><div class="metric"><span>Esercizi con storico</span><b>${s.exercise_trends||0}</b></div><div class="metric"><span>Essenziali aperti</span><b>${s.pending_essentials||0}</b></div><div class="metric"><span>Diario alimentare</span><b>${s.nutrition_logged_days_7d||0}/7 gg</b></div><div class="metric"><span>Health Auto Export</span><b class="${s.health_auto_export_connected?'green':''}">${s.health_auto_export_connected?'Attivo':'Non rilevato'}</b></div><div class="metric"><span>Peso da iPhone</span><b>${s.health_auto_export_weight??'—'}${s.health_auto_export_weight!=null?' kg':''}</b></div></div><div class="sub" style="margin-top:10px">Il Coach usa lo storico, la progressione, il recupero, il diario e i dati disponibili da Health Auto Export. Non viene inviato il database grezzo.</div>`;
