...
(function(){
  ...
  function advanceFrom(id){setActive(null);const all=cards(),i=all.findIndex(c=>idOf(c)===id),next=all.slice(i+1).find(c=>!isArchived(idOf(c),c))||all.find(c=>!isArchived(idOf(c),c));if(next)ensureStarted(idOf(next));else restoreActive();try{window.if67Refresh?.()}catch(e){}}
